...
- For morning Scripture, accept only a verified catalog identifier and an
  optional plain-text connection sentence from `content.json`. Supply exact BSB
  text, reference, translation label, and Bible link from the repository-owned
  catalog, never from model-authored fields.
