...
New reader intake is paused by the founder; no bake or sync may fetch new rows.
The committed copy is the source for the whole bake: `--plan` never refreshes or
mutates it.
...
- Spark is the primary Pacific-time clock for the approved bakes: morning at
  ...
  no-ops. Counter Sync remains unauthorized while new reader intake is paused;
  its existing schedule is implementation state to reconcile, not permission to
  invoke it.
