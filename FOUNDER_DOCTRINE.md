...
- David narrowed the proposed zero-dollar, 30-calendar-day outreach learning
  campaign on 2026-08-11 to a lean core-proof campaign. The evening Field Guide
  is the acquisition franchise. The retained lanes are individually approved
  X source cards derived from exact evening editions and up to four qualified
  manual replies on staffed days. Gemini Omni, YouTube, and generated media are
  deferred. The campaign clock starts only after one privacy-safe observable
  return path is implemented, validated, recorded, and included in an exact
  activation receipt approved by David. Every source card, reply, account
  change, deployment, and spend remains separately gated under
...
- Cloudflare Workers + D1 is selected only for provider-specific local audience
  ...
  that selection.
- The outreach campaign keeps RSS as a canonical delivery surface and manual
  X-native aggregate diagnostics as a zero-cost pre-baseline fallback, but
  unobservable RSS intent cannot start the campaign clock. One privacy-safe
  observable return path must first be implemented and validated under a
  separate reviewed change. This does not reopen email, activate analytics or
  community features, authorize generated media, or satisfy the official
  first-1,000 measurement definition. Site return remains unknown.
